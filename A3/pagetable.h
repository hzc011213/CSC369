...
typedef struct pt_entry_s {
    union {
        struct {
            int frame_number;  // Physical page frame number (4th level)
        };
        struct {
            struct pt_level_s *next_level; // Pointer to next level table (1st to 3rd level)
        };
    };
    int swap_offset;       // Swap offset if the page is evicted
    unsigned char valid;        // Valid bit flag: 1 if valid, 0 if invalid
    unsigned char dirty;        // Dirty bit flag: 1 if dirty, 0 if not dirty
    unsigned char referenced;   // Referenced bit flag: 1 if referenced, 0 if not
} pt_entry_t;
